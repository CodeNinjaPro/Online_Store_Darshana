...
function selectedRowToInput() {
	...
	for (var i = 1; i < table.rows.length; i++) {
		table.rows[i].onclick = function () {
			...
			document.getElementById("contact").value = this.cells[4].textContent;
			document.getElementById("loyality").value = this.cells[5].textContent;
...
function save() {
	...
	var loyality = $('#loyality').val();
	var date_time = $('#date_time').val();
	var action = "insert";
	if(customer_id === "" || name === "" || address === "" || contact === "" || loyality === "" || date_time === ""){
		alert("Please Enter All Details")
	} else {
		$.ajax({
			url: 'CustomerServlet',
			method: 'POST',
			data: {action: action, customer_id: customer_id, name: name, address: address, contact: contact, loyality: loyality, date_time: date_time},
...
function update() {
	...
	} else if(customer_id === "" || name === "" || address === "" || contact === "" || loyality === "" || date_time === ""){
		alert("Please Enter All Details")
	} else {
		$.ajax({
			url: 'CustomerServlet',
